 ...
 */
import { tokenize } from './gdscript-scanner.js';
/**
 ...
 */
export function classifyFirstArgument(tokens, openParenIndex) {
    let depth = 0;
    const collected = [];
    for (let j = openParenIndex + 1; j < tokens.length; j++) {
        ...
        if (tok.kind === 'punct') {
            ...
            if (tok.text === ')') {
                if (depth === 0)
                    break; // terminator: end of the call
                depth--;
                collected.push(tok);
                continue;
            }
    ...
    if (collected.length === 0)
        return 'none';
    if (collected.length === 1 && collected[0].kind === 'string')
        return 'literal';
    return 'nonliteral';
}
// ---------------------------------------------------------------------------
...
export const policyRules = [
    ...
    },
    {
        ...
        reason: 'Callable(target, "method") constructs runtime dynamic dispatch that bypasses static analysis',
    ...
    },
    {
        ...
        argumentKind: 'nonliteral',
        reason: 'ProjectSettings.call with a non-literal method name bypasses the ProjectSettings.* allowlist',
    ...
    // reflexive elicitation approval.
    {
        ...
        argumentKind: 'nonliteral',
        reason: 'A non-literal .call on an arbitrary receiver is dynamic dispatch that bypasses static analysis',
        solutions: [
    ...
    },
    {
        ...
        matchLastSegment: true,
        argumentKind: 'nonliteral',
        reason: 'A non-literal .callv on an arbitrary receiver is dynamic dispatch that bypasses static analysis',
        solutions: ['Call the method directly by name'],
    },
];
// ---------------------------------------------------------------------------
 ...
 */
function indexOfOpenParen(tokens, i) {
...
function tokenMatchesRule(tok, rule) {
    ...
    if (tok.kind !== 'memberChain' || !tok.chain)
        return false;
    if (tok.chain.length < rule.chain.length)
        return false;
    for (let i = 0; i < rule.chain.length; i++) {
        if (tok.chain[i] !== rule.chain[i])
            return false;
    }
    return true;
}
export function evaluateScript(source, strict = false) {
    const tokens = tokenize(source);
    const matches = [];
    let promotedByStrict = false;
    for (let i = 0; i < tokens.length; i++) {
        const tok = tokens[i];
        if (tok.kind !== 'identifier' && tok.kind !== 'memberChain')
            continue;
        for (const rule of policyRules) {
            if (!tokenMatchesRule(tok, rule))
                continue;
            ...
            if (rule.argumentKind) {
                ...
                if (classification !== rule.argumentKind)
                    continue;
            }
    ...
    for (const m of matches) {
        if (highest === null || m.tier < highest)
            highest = m.tier;
    }
    let decision = 'ok';
    if (highest === 1)
 ...
 */
export function matchesToWarnings(matches) {
    return matches.map((m) => `Warning line ${m.line}: ${m.matchedText} — ${m.reason}`);
}
//# sourceMappingURL=run-script-policy.js.map