 ...
 */
function readSceneFileSafe(scenePath) {
    try {
 ...
 */
function extractExtResourcePaths(content, typeValue, requiredExt) {
    const result = [];
 ...
 */
export function readMainSceneFromProject(projectDir) {
 ...
 * stale.
 */
export function resolveLaunchScene(projectDir, sceneArg) {
    if (sceneArg) {
 ...
 */
export function extractSceneScripts(scenePath, projectDir) {
    ...
    // `path="res://..."` clause.
    return extractExtResourcePaths(content, 'Script', '.gd').map((stripped) => join(projectDir, stripped));
}
/**
 ...
 */
export function collectSceneScriptsRecursive(scenePath, projectDir) {
    const visited = new Set();
    const scripts = new Set();
    function walk(currentScenePath) {
        const absScenePath = resolve(currentScenePath);
        if (visited.has(absScenePath))
            return;
...
//# sourceMappingURL=scene-parsing.js.map