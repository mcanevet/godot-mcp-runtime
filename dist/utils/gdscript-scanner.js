 ...
 * branch in `tokenize`).
 */
function skipWsAndNewlines(source, len, pos, line, lineStart) {
    while (pos < len) {
        const c = source[pos];
        if (c === ' ' || c === '\t') {
            pos++;
...
export function tokenize(source) {
    ...
    let line = 1;
    let lineStart = 0;
    const colOf = (pos) => pos - lineStart + 1;
    while (i < len) {
        const ch = source[i];
        ...
        if (ch === '$') {
            ...
            if (i < len && (source[i] === '"' || source[i] === "'")) {
                ...
                while (i < len && isNodePathChar(source[i]))
                    i++;
            }
        ...
        if (isIdentStart(ch)) {
            ...
            while (i < len) {
                ...
                if (beforeDot.pos >= len || source[beforeDot.pos] !== '.')
                    break;
                const afterDot = skipWsAndNewlines(source, len, beforeDot.pos + 1, beforeDot.line, beforeDot.lineStart);
                if (afterDot.pos >= len || !isIdentStart(source[afterDot.pos]))
                    break;
                let k = afterDot.pos;
                while (k < len && isIdentPart(source[k]))
                    k++;
 ...
 */
export function tokenizeStripped(source) {
    return tokenize(source).filter((t) => t.kind !== 'newline');
}
//# sourceMappingURL=gdscript-scanner.js.map