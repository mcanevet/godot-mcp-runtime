 ...
 */
function createContextFromServer(server) {
    const elicitor = async (request) => {
...
class GodotMcpServer {
    ...
    constructor(config) {
        ...
        if (process.env.GODOT_MCP_DISABLE_ELICITATION === 'true' && this.ctx.strictMode) {
            console.error('[SERVER] GODOT_MCP_DISABLE_ELICITATION ignored: strict mode requires explicit confirmation');
        }
        else if (this.ctx.disableElicitation) {
            console.error('[SERVER] Elicitation disabled (GODOT_MCP_DISABLE_ELICITATION=true); confirmation prompts auto-accepted');
        }
