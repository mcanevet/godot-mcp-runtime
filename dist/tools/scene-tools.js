...
import { parseProjectArgs, parseSceneArgs, parseRequiredNodePath, requireString, optionalString, optionalStringArray, optionalBoolean, requireArray, optionalObject, } from '../utils/arg-parsing.js';
...
export async function handleCreateScene(runner, args) {
    ...
    return executeSceneOp(runner, 'create_scene', params, parsed.value.projectPath, 'Failed to create scene', ['Check if the root node type is valid'], undefined, { parseStdoutAsJson: true });
}
export async function handleAddNode(runner, args) {
    ...
    const mergedProps = { ...(properties.value ?? {}) };
    ...
    if (args.parentNodePath !== undefined) {
        ...
        if (!parentNodePath.ok)
            return parentNodePath;
        params.parentNodePath = parentNodePath.value;
    }
    if (Object.keys(mergedProps).length > 0)
        params.properties = mergedProps;
    return executeSceneOp(runner, 'add_node', params, parsed.value.projectPath, 'Failed to add node', ['Check if the node type is valid', 'Ensure the parent node path exists']);
}
export async function handleLoadSprite(runner, args) {
    ...
    if (!validateSubPath(parsed.value.projectPath, texturePath.value)) {
        return err(createErrorResponse('Valid texturePath is required', [
    ...
    if (!existsSync(textureFullPath)) {
        return err(createErrorResponse(`Texture file does not exist: ${texturePath.value}`, [
    ...
    return executeSceneOp(runner, 'load_sprite', params, parsed.value.projectPath, 'Failed to load sprite', ['Check if the node is a Sprite2D, Sprite3D, or TextureRect']);
}
export async function handleSaveScene(runner, args) {
    ...
    if (newPath.value && !validateSubPath(parsed.value.projectPath, newPath.value)) {
        return err(createErrorResponse('Invalid newPath', [
    ...
    if (newPath.value)
        params.newPath = newPath.value;
    return executeSceneOp(runner, 'save_scene', params, parsed.value.projectPath, 'Failed to save scene', ['Check if the scene file is valid']);
}
export async function handleExportMeshLibrary(runner, args) {
    ...
    if (!validateSubPath(parsed.value.projectPath, outputPath.value)) {
        return err(createErrorResponse('Valid outputPath is required', [
    ...
    return executeSceneOp(runner, 'export_mesh_library', params, parsed.value.projectPath, 'Failed to export mesh library', ['Check if the scene contains valid 3D meshes']);
}
export async function handleBatchSceneOperations(runner, args) {
    ...
    return executeSceneOp(runner, 'batch_scene_operations', params, parsed.value.projectPath, 'Batch scene operations failed', ['Check that all scene paths exist', 'Ensure node types are valid'], undefined, { parseStdoutAsJson: true });
}
