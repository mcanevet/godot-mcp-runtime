...
import { parseSceneArgs, parseRequiredNodePath, parseOptionalNodePath, parseNodePath, requireString, optionalString, requireStringArray, requireArray, optionalNumber, optionalBoolean, } from '../utils/arg-parsing.js';
...
export async function handleDeleteNodes(runner, args) {
    ...
    return executeSceneOp(runner, 'delete_nodes', params, parsed.value.projectPath, 'Failed to delete nodes', ['Check if the node paths are correct'], undefined, { parseStdoutAsJson: true });
}
export async function handleSetNodeProperties(runner, args) {
    ...
    return executeSceneOp(runner, 'set_node_properties', params, parsed.value.projectPath, 'Failed to set node properties', ['Check node paths and property names'], undefined, { parseStdoutAsJson: true });
}
export async function handleGetNodeProperties(runner, args) {
    ...
    return executeSceneOp(runner, 'get_node_properties', params, parsed.value.projectPath, 'Failed to get node properties', ['Check node paths']);
}
export async function handleAttachScript(runner, args) {
    ...
    if (!validateSubPath(parsed.value.projectPath, scriptPath.value)) {
        return err(createErrorResponse('Valid scriptPath is required', [
    ...
    if (!existsSync(scriptFullPath)) {
        return err(createErrorResponse(`Script file does not exist: ${scriptPath.value}`, [
    ...
    return executeSceneOp(runner, 'attach_script', params, parsed.value.projectPath, 'Failed to attach script', ['Ensure the script is valid for this node type'], undefined, { parseStdoutAsJson: true });
}
export async function handleGetSceneTree(runner, args) {
    ...
    if (!maxDepth.ok)
        return maxDepth;
    const params = { scenePath: parsed.value.scenePath };
    if (parentPath.value)
        params.parentPath = parentPath.value;
    if (maxDepth.value !== undefined)
        params.maxDepth = maxDepth.value;
    return executeSceneOp(runner, 'get_scene_tree', params, parsed.value.projectPath, 'Failed to get scene tree', ['Ensure the scene is valid']);
}
export async function handleDuplicateNode(runner, args) {
    ...
    if (newName.value)
        params.newName = newName.value;
    if (targetParentPath.value)
        params.targetParentPath = targetParentPath.value;
    return executeSceneOp(runner, 'duplicate_node', params, parsed.value.projectPath, 'Failed to duplicate node', ['Check if the node path and target parent path are correct'], undefined, { parseStdoutAsJson: true });
}
export async function handleGetNodeSignals(runner, args) {
    ...
    return executeSceneOp(runner, 'get_node_signals', params, parsed.value.projectPath, 'Failed to get node signals', ['Check if the node path is correct'], undefined, { parseStdoutAsJson: true });
}
function parseSignalArgs(args) {
    const parsed = parseSceneArgs(args);
    if (!parsed.ok)
        return parsed;
    const nodePath = parseRequiredNodePath(args, 'nodePath');
...
export async function handleConnectSignal(runner, args) {
    ...
    return executeSceneOp(runner, 'connect_signal', params, parsed.value.projectPath, 'Failed to connect signal', ['Ensure the signal exists on the source node and the method exists on the target node']);
}
export async function handleDisconnectSignal(runner, args) {
    ...
    return executeSceneOp(runner, 'disconnect_signal', params, parsed.value.projectPath, 'Failed to disconnect signal', ['Ensure the signal connection exists before trying to disconnect it']);
}
