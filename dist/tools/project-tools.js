...
import { parseProjectArgs, parseSceneArgs, requireString, optionalString, optionalBoolean, optionalNumber, optionalStringArray, } from '../utils/arg-parsing.js';
...
export async function handleListProjects(args) {
    ...
    if (!validatePath(directory.value)) {
        return err(createErrorResponse('Invalid directory path', [
            'Provide a valid path without ".." or other potentially unsafe characters',
        ]));
    }
    try {
        if (!existsSync(directory.value)) {
            return err(createErrorResponse(`Directory does not exist: ${directory.value}`, [
        ...
        return err(createErrorResponse(`Failed to list projects: ${getErrorMessage(error)}`, [
...
export async function handleGetProjectInfo(runner, args) {
    ...
    try {
        ...
        return err(createErrorResponse(`Failed to get project info: ${getErrorMessage(error)}`, [
...
export async function handleGetProjectFiles(args) {
    ...
    try {
        ...
        return err(createErrorResponse(`Failed to get project files: ${getErrorMessage(error)}`, [
...
export async function handleSearchProject(args) {
    ...
    try {
        ...
        const result = searchInFiles(parsed.value.projectPath, pattern.value, fileTypes, caseSensitive, maxResults);
        return createStructuredResponse(result);
    }
    catch (error) {
        return err(createErrorResponse(`Failed to search project: ${getErrorMessage(error)}`, [
            'Check if the project directory is accessible',
        ]));
    }
}
export async function handleGetSceneDependencies(args) {
    ...
    try {
        ...
        return err(createErrorResponse(`Failed to get scene dependencies: ${getErrorMessage(error)}`, [
...
export async function handleGetProjectSettings(args) {
    ...
    try {
        ...
        return err(createErrorResponse(`Failed to get project settings: ${getErrorMessage(error)}`, [
