...
import { validateSubPath, isUnderDir, projectGodotPath, stripResPrefix, } from '../utils/path-validation.js';
...
import { parseProjectArgs, optionalString, optionalNumber, optionalBoolean, requireString, requireArray, } from '../utils/arg-parsing.js';
import { ok, err } from '../utils/result.js';
import { logDebug } from '../utils/logger.js';
import { randomUUID } from 'crypto';
import { createNullContext, normalizeProjectKey, } from '../utils/mcp-context.js';
import { evaluateScript, matchesToWarnings, summarizeMatch, } from '../utils/run-script-policy.js';
...
function formatMoreFindingsSuffix(total) {
    if (total <= 1)
        return '';
    const extra = total - 1;
    return ` (+${extra} more finding${extra > 1 ? 's' : ''})`;
}
function isElicitAccepted(result) {
    return (result.action === 'accept' && (result.content === undefined || result.content.confirm === true));
}
/**
 ...
 * the frame.
 */
function parseBridgeJson(responseStr, context) {
    try {
        return ok(JSON.parse(responseStr));
    }
    catch (error) {
        return err(createErrorResponse(`Invalid response from bridge (${context}): ${getErrorMessage(error)}`, [
 ...
 */
function attachRuntimeWarnings(target, runtimeErrors) {
 ...
 */
function writeAuditSidecar(projectPath, script, decision, policy, strictMode) {
    try {
        const projectRoot = resolve(projectPath);
        const scriptsDir = resolve(join(projectRoot, '.mcp', 'scripts'));
        if (!isUnderDir(projectRoot, scriptsDir)) {
            logDebug(`Sidecar write skipped: resolved script dir ${scriptsDir} escapes projectRoot ${projectRoot}`);
            return;
        }
 ...
 */
function formatBlockMessage(matches) {
    if (matches.length === 0)
        return 'Blocked by run_script security policy.';
...
function collectSolutions(matches) {
    const seen = new Set();
    const out = [];
    for (const m of matches) {
        ...
        if (out.length >= MAX_POLICY_SOLUTIONS)
            break;
    }
    return out;
}
 ...
 */
function formatScanFinding(sourcePath, projectPath, match) {
 ...
 */
function scanScriptFile(filePath, strict) {
    let source;
    try {
...
function ensureRuntimeSession(runner, actionDescription) {
    if (!runner.activeSessionMode || !runner.activeProjectPath) {
        return err(createErrorResponse(`No active runtime session. A project must be running or attached to ${actionDescription}.`, [
    ...
    if (runner.activeSessionMode === 'spawned' &&
        ...
        return err(createErrorResponse(`The spawned Godot process has exited and cannot ${actionDescription}.`, [
...
export async function handleLaunchEditor(runner, args) {
    ...
    try {
        if (!runner.getGodotPath()) {
            ...
            if (!runner.getGodotPath()) {
                return err(createErrorResponse('Could not find a valid Godot executable path', [
        ...
        return err(createErrorResponse(`Failed to launch Godot editor: ${getErrorMessage(error)}`, [
            'Ensure Godot is installed correctly',
            'Check if the GODOT_PATH environment variable is set correctly',
        ]));
    }
}
export async function handleRunProject(runner, args, ctx = createNullContext()) {
    args = normalizeParameters(args);
    const parsed = parseProjectArgs(args);
    if (!parsed.ok)
        return parsed;
    ...
    if (scene.value !== undefined) {
        if (!validateSubPath(projectPath, scene.value)) {
            return err(createErrorResponse(`Invalid scene path: must be project-relative without ".." (got: ${scene.value})`, ['Pass scene as a path relative to the project root, e.g. "scenes/main.tscn"']));
        }
    }
    ...
    try {
        ...
        if (existsSync(projectGodot)) {
            ...
            for (const entry of autoloads) {
                ...
                if (!stripped.endsWith('.gd'))
                    continue;
                if (!validateSubPath(absProjectPath, stripped)) {
                    scanWarnings.push(`Skipped autoload ${entry.name}: path "${entry.path}" escapes project root.`);
                    continue;
                }
        ...
        if (launchScene === null) {
            scanWarnings.push('No launchable scene found (no `run/main_scene` and no explicit scene arg); scene-script scan skipped.');
        }
        else if (!existsSync(launchScene)) {
            scanWarnings.push(`Configured launch scene not found at ${launchScene}; scene-script scan skipped.`);
        }
        else {
    ...
    if (ctx.strictMode && hasTier1) {
        ...
        const more = scanFindings.length > top.length ? ` (+${scanFindings.length - top.length} more)` : '';
        return err(createErrorResponse([
    ...
    if (!ctx.sessionState.runProjectConfirmed.has(projectKey)) {
        if (ctx.disableElicitation) {
            ...
            scanWarnings.push('Elicitation disabled (GODOT_MCP_DISABLE_ELICITATION); launching without user confirmation.');
            ...
            try {
                elicitResult = await ctx.elicitor({
                    message: 'Launching a Godot project executes arbitrary code in its autoloads and main scene. Proceed?',
                    requestedSchema: {
                        type: 'object',
                        properties: {
                ...
                if (ctx.strictMode) {
                    return err(createErrorResponse(`${elicitMsg}; strict mode refuses to launch without explicit user confirmation.`, [
            ...
            if (!isElicitAccepted(elicitResult)) {
                ...
                return err(createErrorResponse(cancelled
    ...
    if (bridgePort.value !== undefined) {
        if (!Number.isInteger(bridgePort.value) || bridgePort.value < 1 || bridgePort.value > 65535) {
            return err(createErrorResponse(`Invalid bridgePort: must be an integer in [1, 65535] (got: ${String(bridgePort.value)})`, ['Omit bridgePort to auto-select a free port', 'Pass a valid TCP port number']));
        }
    }
    ...
    try {
        ...
        if (!bridgeResult.ready) {
            if (runner.activeProcess && runner.activeProcess.hasExited) {
                ...
                return err(createErrorResponse(`Godot process exited before the MCP bridge could initialize.\n${bridgeResult.error || ''}`, [
        ...
        if (allWarnings.length > 0) {
            ...
            for (const w of allWarnings.slice(0, MAX_SCAN_WARNINGS_SHOWN))
                lines.push(`- ${w}`);
        ...
        if (errorMessage.includes('No display server available')) {
            return err(createErrorResponse(`Failed to run Godot project: ${errorMessage}`, [
...
export async function handleAttachProject(runner, args) {
    ...
    if (!attachBridgePort.ok)
        return attachBridgePort;
    if (attachBridgePort.value !== undefined) {
        if (!Number.isInteger(attachBridgePort.value) ||
            attachBridgePort.value < 1 ||
            attachBridgePort.value > 65535) {
            return err(createErrorResponse(`Invalid bridgePort: must be an integer in [1, 65535] (got: ${String(attachBridgePort.value)})`, [
    ...
    try {
        ...
        if (!bridgeResult.ready) {
            ...
            return err(createErrorResponse(`${racePrefix}Project attached but the MCP bridge is not ready.\n${bridgeResult.error || ''}`, solutions));
        }
        ...
        return err(createErrorResponse(`Failed to attach project: ${getErrorMessage(error)}`, [
...
export async function handleDetachProject(runner) {
    if (runner.activeSessionMode !== 'attached') {
        return err(createErrorResponse('No attached project to detach.', [
...
export function handleGetDebugOutput(runner, args = {}) {
    ...
    if (!runner.activeSessionMode) {
        return err(createErrorResponse('No active runtime session.', [
            'Use run_project to start a Godot project first',
            'Or use attach_project before launching Godot manually',
        ]));
    }
    ...
    if (!proc) {
        return err(createErrorResponse('No active spawned process is available for debug output.', [
            'Use run_project to start a Godot project first',
...
export async function handleStopProject(runner) {
    const result = await runner.stopProject();
    if (!result) {
        return err(createErrorResponse('No active Godot process to stop.', [
            'Use run_project to start a Godot project first',
...
export async function handleTakeScreenshot(runner, args) {
    ...
    if (responseMode === null) {
        return err(createErrorResponse('Invalid responseMode for take_screenshot', [
    ...
    if (previewMaxWidth === null || previewMaxHeight === null) {
        return err(createErrorResponse('Invalid preview dimensions for take_screenshot', [
    ...
    try {
        ...
        if (parsed.error) {
            return err(createErrorResponse(`Screenshot server error: ${parsed.error}`, [
        ...
        if (!parsed.path) {
            return err(createErrorResponse('Screenshot server returned no file path', [
        ...
        if (!isUnderDir(screenshotsRoot, screenshotPath)) {
            return err(createErrorResponse('Bridge returned a screenshot path outside .mcp/screenshots/. Refusing to read.', [
        ...
        if (!existsSync(screenshotPath)) {
            return err(createErrorResponse(`Screenshot file not found at: ${screenshotPath}`, [
        ...
        if (responseMode === 'full') {
            ...
            if (!parsed.preview_path) {
                return err(createErrorResponse('Screenshot server returned no preview path', [
            ...
            if (!isUnderDir(screenshotsRoot, previewPath)) {
                return err(createErrorResponse('Bridge returned a screenshot preview path outside .mcp/screenshots/. Refusing to read.', [
                    'This indicates a tampered or misbehaving McpBridge autoload',
                    'Stop the project, verify the bridge script is the one shipped with this server, and retry',
                ]));
            }
            if (!existsSync(previewPath)) {
                return err(createErrorResponse(`Screenshot preview file not found at: ${previewPath}`, [
        ...
        return err(createErrorResponse(`Failed to take screenshot: ${getErrorMessage(error)}`, [
...
export async function handleSimulateInput(runner, args) {
    ...
    try {
        ...
        const parsed = parsedResult.value;
        if (parsed.error) {
            return err(createErrorResponse(`Input simulation error: ${parsed.error}`, [
        ...
        return err(createErrorResponse(`Failed to simulate input: ${getErrorMessage(error)}`, [
...
export async function handleGetUiElements(runner, args) {
    ...
    try {
        ...
        if (filterResult.value)
            cmdParams.type_filter = filterResult.value;
        ...
        const parsed = parsedResult.value;
        if (parsed.error) {
            return err(createErrorResponse(`UI element query error: ${parsed.error}`, [
        ...
        return err(createErrorResponse(`Failed to get UI elements: ${getErrorMessage(error)}`, [
            'Check get_debug_output for crash backtraces or runtime errors',
            'If the game has exited, call stop_project, then run_project again',
        ]));
    }
}
export async function handleRunScript(runner, args, ctx = createNullContext()) {
    args = normalizeParameters(args);
    ...
    if (!script.includes('func execute')) {
        return err(createErrorResponse('Script must define func execute(scene_tree: SceneTree) -> Variant', [
    ...
    if (policy.decision === 'hard_block') {
        ...
        return err(createErrorResponse(formatBlockMessage(policy.matches), collectSolutions(policy.matches)));
    }
    ...
    if (policy.decision === 'elicit_required') {
        if (ctx.disableElicitation) {
            ...
            try {
                ...
                return err(createErrorResponse(`Elicitation unavailable: ${policy.matches[0]?.matchedText ?? 'Tier 2 primitive'} requires user confirmation but the client does not support elicitation. Cause: ${getErrorMessage(error)}`, [
            ...
            if (!isElicitAccepted(elicitResult)) {
                ...
                return err(createErrorResponse(`User declined: ${summarizeMatch(policy.matches[0])}. The script was not executed.`, collectSolutions(policy.matches)));
            }
    ...
    try {
        ...
        const parsed = parsedResult.value;
        if (parsed.error) {
            return err(createErrorResponse(`Script execution error: ${parsed.error}`, [
        ...
        if (parsed.success && parsed.result === null && runner.activeSessionMode === 'spawned') {
            if (runtimeErrors.length > 0) {
                ...
                return err(createErrorResponse(`Script runtime error detected:\n${errorContext}`, [
        ...
        return err(createErrorResponse(`Failed to execute script: ${getErrorMessage(error)}`, [
